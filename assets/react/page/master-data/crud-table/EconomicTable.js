...
export default function EconomicTable() {
    ...
    const getProvince = async () => {
        return await EconomicApi.getProvince();
    }
    const getYear = async () => {
        return await EconomicApi.getYear();
    }
    const getFactor = async () => {
        return await EconomicApi.getFactor();
    }
    const downloadTemplate = async () => {
        let template = await MasterDataApi.downloadExcel('economic');
        FileDownload(template.data,'economic_template.xlsx');
    }
    ...
    const handleUpload = async () => {
        ...
        try {
            let res = await EconomicApi.import(formData);
            setFile(null);
    ...
    const searchData = async () => {
        let val = await EconomicApi.search(search)
        setRows(val.data);
    }
    const columns = [
    ...
    const isEnabledHandleChange = async (isEnabled, id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                await EconomicApi.setIsEnabled({isEnabled:isEnabled,id:id,email:identity.email});
                await searchData();
    ...
    const updateVal = async () => {
        Swal.fire({
            ...
            setLoading(true);
            if (result.isConfirmed) {
                await EconomicApi.update(editVal);
                await searchData();
                await resetEditVal();
            } else if (result.dismiss === Swal.DismissReason.cancel){
                await resetEditVal();
