...
import HumanDevelopmentApi from "../../../api/HumanDevelopmentApi";
...
export default function HumanDevelopmentTable() {
    ...
    const getProvince = async () => {
        return await HumanDevelopmentApi.getProvince();
    }
    const getYear = async () => {
        return await HumanDevelopmentApi.getYear();
    }
    const getFactor = async () => {
        return await HumanDevelopmentApi.getFactor();
    }
    const downloadTemplate = async () => {
        let template = await MasterDataApi.downloadExcel('humanDevelopment');
        FileDownload(template.data,'human_development_template.xlsx');
    }
    ...
    const handleUpload = async () => {
        ...
        try {
            let res = await HumanDevelopmentApi.import(formData);
            setFile(null);
    ...
    const searchData = async () => {
        let val = await HumanDevelopmentApi.search(search)
        setRows(val.data);
    }
    const columns = [
    ...
    const isEnabledHandleChange = async (isEnabled, id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                await HumanDevelopmentApi.setIsEnabled({isEnabled:isEnabled,id:id,email:identity.email});
                await searchData();
    ...
    const updateVal = async () => {
        Swal.fire({
            ...
            setLoading(true);
            if (result.isConfirmed) {
                await HumanDevelopmentApi.update(editVal);
                await searchData();
                await resetEditVal();
            } else if (result.dismiss === Swal.DismissReason.cancel){
                await resetEditVal();
