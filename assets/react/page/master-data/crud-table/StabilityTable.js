...
export default function StabilityTable() {
    ...
    const getProvince = async () => {
        return await StabilityApi.getProvince();
    }
    const getYear = async () => {
        return await StabilityApi.getYear();
    }
    const getFactor = async () => {
        return await StabilityApi.getFactor();
    }
    const downloadTemplate = async () => {
        let template = await MasterDataApi.downloadExcel('stability');
        FileDownload(template.data,'stability_template.xlsx');
    }
    ...
    const handleUpload = async () => {
        ...
        try {
            let res = await StabilityApi.import(formData);
            setFile(null);
    ...
    const searchData = async () => {
        let val = await StabilityApi.search(search)
        setRows(val.data);
    }
    const columns = [
    ...
    const isEnabledHandleChange = async (isEnabled, id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                await StabilityApi.setIsEnabled({isEnabled:isEnabled,id:id,email:identity.email});
                await searchData();
    ...
    const updateVal = async () => {
        Swal.fire({
            ...
            setLoading(true);
            if (result.isConfirmed) {
                await StabilityApi.update(editVal);
                await searchData();
                await resetEditVal();
            } else if (result.dismiss === Swal.DismissReason.cancel){
                await resetEditVal();
        ...
        <React.Fragment>
            ...
            >
                <Grid container spacing={2} sx={{ pt: 2, pb: '20px', pl: 2, pr: 2 }}>
                    ...
                    <Grid item xs={12} sm={4} md={4} lg={4} sx={{ marginLeft: '24px', marginRight: '24px' }}>
                        <Grid container direction="row">
                            <Typography variant="subtitle1">ระดับผลกระทบ1</Typography><Typography sx={{ color: 'red' }}>*</Typography>
                        </Grid>
                        <TextField
                            fullWidth
                            size="small"
                            value={editVal.value1}
                            onChange={(e) => {
                    ...
                    <Grid item xs={12} sm={4} md={4} lg={4} sx={{ marginLeft: '24px', marginRight: '24px' }}>
                        <Grid container direction="row">
                            <Typography variant="subtitle1">ระดับผลกระทบ2</Typography><Typography sx={{ color: 'red' }}>*</Typography>
                        </Grid>
                        <TextField
                            fullWidth
                            size="small"
                            value={editVal.value2}
                            onChange={(e) => {
                                let newValue = e.target.value;
                                setEditVal({
                                    ...editVal,
                    ...
                    <Grid item xs={12} sm={4} md={4} lg={4} sx={{ marginLeft: '24px', marginRight: '24px' }}>
                        <Grid container direction="row">
                            <Typography variant="subtitle1">ระดับผลกระทบ3</Typography><Typography sx={{ color: 'red' }}>*</Typography>
                        </Grid>
                        <TextField
                            fullWidth
                            size="small"
                            value={editVal.value3}
                            onChange={(e) => {
                                let newValue = e.target.value;
                                setEditVal({
                                    ...editVal,
                    ...
                    <Grid item xs={12} sm={4} md={4} lg={4} sx={{ marginLeft: '24px', marginRight: '24px' }}>
                        <Grid container direction="row">
                            <Typography variant="subtitle1">ระดับผลกระทบ4</Typography><Typography sx={{ color: 'red' }}>*</Typography>
                        </Grid>
                        <TextField
                            fullWidth
                            size="small"
                            value={editVal.value4}
                            onChange={(e) => {
                                let newValue = e.target.value;
                                setEditVal({
                                    ...editVal,
                    ...
                    <Grid item xs={12} sm={4} md={4} lg={4} sx={{ marginLeft: '24px', marginRight: '24px' }}>
                        <Grid container direction="row">
                            <Typography variant="subtitle1">ระดับผลกระทบ5</Typography><Typography sx={{ color: 'red' }}>*</Typography>
                        </Grid>
                        <TextField
                            fullWidth
                            size="small"
                            value={editVal.value5}
                            onChange={(e) => {
                                let newValue = e.target.value;
                                setEditVal({
                                    ...editVal,
