...
export default function HousesTable(){
    ...
    const getProvince = async () => {
        return await HousesApi.getProvince();
    }

    const getYear = async () => {
        return await HousesApi.getYear();
    }

    const downloadTemplate = async () => {
        let template = await MasterDataApi.downloadExcel('houses');
        FileDownload(template.data,'houses_template.xlsx');
    }
    ...
    const handleUpload = async () => {
        ...
        try {
            let res = await HousesApi.import(formData);
            setFile(null);
    ...
    const searchData = async () => {
        let val = await HousesApi.search(search)
        setRows(val.data);
    }
    ...
    const isEnabledHandleChange = async (isEnabled, id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                await HousesApi.setIsEnabled({isEnabled:isEnabled,id:id,email:identity.email});
                await searchData();
    ...
    const updateVal = async () => {
        Swal.fire({
            ...
            setLoading(true);
            if (result.isConfirmed) {
                await HousesApi.update(editVal);
                await searchData();
                await resetEditVal();
            } else if (result.dismiss === Swal.DismissReason.cancel){
                await resetEditVal();
        ...
        <React.Fragment>
            ...
            >
                <Grid container spacing={2} sx={{ pt: 2, pb: '20px', pl: 2, pr: 2 }}>
                    ...
                    <Grid item xs={12} sm={4} md={4} lg={4} sx={{ marginLeft: '24px', marginRight: '24px' }}>
                        <Grid container direction="row">
                            <Typography variant="subtitle1">จำนวนบ้านเรือน</Typography><Typography sx={{ color: 'red' }}>*</Typography>
                        </Grid>
                        <TextField
                            fullWidth
                            size="small"
