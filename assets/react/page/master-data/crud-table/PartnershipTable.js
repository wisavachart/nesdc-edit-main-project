...
export default function PartnershipTable() {
    ...
    const getProvince = async () => {
        return await PartnershipApi.getProvince();
    }
    const getYear = async () => {
        return await PartnershipApi.getYear();
    }
    const getFactor = async () => {
        return await PartnershipApi.getFactor();
    }
    const downloadTemplate = async () => {
        let template = await MasterDataApi.downloadExcel('partnership');
        FileDownload(template.data, 'partnership_template.xlsx');
    }
    ...
    const handleUpload = async () => {
        ...
        try {
            let res = await PartnershipApi.import(formData);
            setFile(null);
    ...
    const searchData = async () => {
        let val = await PartnershipApi.search(search)
        setRows(val.data);
    }
    ...
    const isEnabledHandleChange = async (isEnabled, id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                await PartnershipApi.setIsEnabled({isEnabled: isEnabled, id: id, email: identity.email});
                await searchData();
    ...
    const updateVal = async () => {
        Swal.fire({
            ...
            setLoading(true);
            if (result.isConfirmed) {
                await PartnershipApi.update(editVal);
                await searchData();
                await resetEditVal();
            } else if (result.dismiss === Swal.DismissReason.cancel) {
                await resetEditVal();
        ...
        <React.Fragment>
            ...
            >
                <Grid container spacing={2} sx={{pt: 2, pb: '20px', pl: 2, pr: 2}}>
                    ...
                    <Grid item xs={12} sm={4} md={4} lg={4} sx={{marginLeft: '24px', marginRight: '24px'}}>
                        <Grid container direction="row">
                            <Typography variant="subtitle1">ความเป็นหุ้นส่วนการพัฒนา</Typography><Typography
