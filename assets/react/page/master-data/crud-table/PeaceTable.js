...
import PeaceApi from "../../../api/PeaceApi";
...
export default function PeaceTable() {
    ...
    const getProvince = async () => {
        return await PeaceApi.getProvince();
    }
    const getYear = async () => {
        return await PeaceApi.getYear();
    }
    const getFactor = async () => {
        return await PeaceApi.getFactor();
    }
    const downloadTemplate = async () => {
        let template = await MasterDataApi.downloadExcel('peace');
        FileDownload(template.data, 'peace_template.xlsx');
    }
    ...
    const handleUpload = async () => {
        ...
        try {
            let res = await PeaceApi.import(formData);
            setFile(null);
    ...
    const searchData = async () => {
        let val = await PeaceApi.search(search)
        setRows(val.data);
    }
    ...
    const isEnabledHandleChange = async (isEnabled, id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                await PeaceApi.setIsEnabled({isEnabled: isEnabled, id: id, email: identity.email});
                await searchData();
        ...
        <React.Fragment>
            ...
            >
                <Grid container spacing={2} sx={{pt: 2, pb: '20px', pl: 2, pr: 2}}>
                    ...
                    <Grid item xs={12} sm={4} md={4} lg={4} sx={{marginLeft: '24px', marginRight: '24px'}}>
                        <Grid container direction="row">
                            <Typography variant="subtitle1">ค่าข้อมูล</Typography><Typography
