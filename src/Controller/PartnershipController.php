...
{
    #[Route('/api/partnership/get-year', name: 'partnership_get_year')]
    ...
    #[Route('/api/partnership/get-provinces', name: 'partnership_get_provinces')]
    public function GetProvinces(EntityManagerInterface $em): Response
    {
        $provinces = $em->getRepository(Partnership::class)->getProvincesByEn();
        $provinceArr = [];
        $provinceArr[] = array(
            'provinceId' => 0,
    ...
    #[Route('/api/partnership/get-factor', name: 'partnership_get_factor')]
    ...
    #[Route('/api/partnership/search', name: 'partnership_search', methods: ['POST'])]
    public function Search(EntityManagerInterface $em, Request $request): Response
    {
        ...
        $en = $em->getRepository(Partnership::class)->findByProvinceAndYear($provinces, $year);
    ...
    #[Route('/api/partnership/mass-upload', name: 'partnership_mass_upload', methods: ['POST'])]
        ...
        if ($user) {
            if ($getFile) {
                ...
                foreach ($rows as $key => $row) {
                    ...
                    $factor = null;
                    if (is_numeric($row[2])) {
                        $factor = $em->getRepository(Factors::class)->findOneBy(['id' => $row[2], 'subGroup' => 7] );
                        if (!$factor) {
                    ...
                    $en = null ;
                    if ($province && $factor) $en = $em->getRepository(Partnership::class)->findOneBy(['province' => $province, 'year' => $row[4], 'factor' => $factor]);
                    if (!$en) {
                        $en = new Partnership();
    ...
    #[Route('/api/partnership/update', name: 'partnership_is_update', methods: ['POST'])]
    public function Update(EntityManagerInterface $em, Request $request): Response
    {
        ...
        if ($email && $factorId) {
            ...
            if ($user && $factor) {
                if ($id) {
                    $val = $em->getRepository(Partnership::class)->find($id);
                    if ($val) {
                        $val->setValue($value);
    ...
    #[Route('/api/partnership/is-enabled', name: 'partnership_is_enabled', methods: ['POST'])]
