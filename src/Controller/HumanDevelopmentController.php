...
class HumanDevelopmentController extends AbstractController
{
    #[Route('/api/human-development/get-year', name: 'human_development_get_year')]
    ...
    #[Route('/api/human-development/get-provinces', name: 'human_development_get_provinces')]
    public function GetProvinces(EntityManagerInterface $em): Response
    {
        $provinces = $em->getRepository(HumanDeverlopment::class)->getProvincesByEn();
        $provinceArr = [];
        $provinceArr[] = array(
            'provinceId' => 0,
    ...
    #[Route('/api/human-development/get-factor', name: 'human_development_get_factor')]
    ...
    #[Route('/api/human-development/search', name: 'human_development_search', methods: ['POST'])]
    public function Search(EntityManagerInterface $em, Request $request): Response
    {
        ...
        $en = $em->getRepository(HumanDeverlopment::class)->findByProvinceAndYear($provinces, $year);
    ...
    #[Route('/api/human-development/mass-upload', name: 'human_development_mass_upload', methods: ['POST'])]
        ...
        if ($user) {
            if ($getFile) {
                ...
                foreach ($rows as $key => $row) {
                    ...
                    $factor = null;
                    if (is_numeric($row[2])) {
                        $factor = $em->getRepository(Factors::class)->findOneBy(['id' => $row[2],'subGroup' => 4]);
                        if (!$factor) {
                    ...
                    $en = null ;
                    if ($province && $factor) $en = $em->getRepository(HumanDeverlopment::class)->findOneBy(['province' => $province, 'year' => $row[4], 'factor' => $factor]);
                    if (!$en) {
                        $en = new HumanDeverlopment();
    ...
    #[Route('/api/human-development/update', name: 'human_development_is_update', methods: ['POST'])]
    public function Update(EntityManagerInterface $em, Request $request): Response
    {
        ...
        if ($email && $factorId) {
            ...
            if ($user && $factor) {
                if ($id) {
                    $val = $em->getRepository(HumanDeverlopment::class)->find($id);
                    if ($val) {
                        $val->setValue($value);
    ...
    #[Route('/api/human-development/is-enabled', name: 'human_development_is_enabled', methods: ['POST'])]
