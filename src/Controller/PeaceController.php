...
class PeaceController extends AbstractController
{
    #[Route('/api/peace/get-year', name: 'peace_get_year')]
    ...
    #[Route('/api/peace/get-provinces', name: 'peace_get_provinces')]
    public function GetProvinces(EntityManagerInterface $em): Response
    {
        $provinces = $em->getRepository(peace::class)->getProvincesByEn();
        $provinceArr = [];
        $provinceArr[] = array(
            'provinceId' => 0,
    ...
    #[Route('/api/peace/get-factor', name: 'peace_get_factor')]
    ...
    #[Route('/api/peace/search', name: 'peace_search', methods: ['POST'])]
    public function Search(EntityManagerInterface $em, Request $request): Response
    {
        ...
        $en = $em->getRepository(Peace::class)->findByProvinceAndYear($provinces, $year);
    ...
    #[Route('/api/peace/mass-upload', name: 'peace_mass_upload', methods: ['POST'])]
        ...
        if ($user) {
            if ($getFile) {
                ...
                foreach ($rows as $key => $row) {
                    ...
                    $factor = null;
                    if (is_numeric($row[2])) {
                        $factor = $em->getRepository(Factors::class)->findOneBy(['id' => $row[2], 'subGroup' => 6]);
                        if (!$factor) {
                    ...
                    $en = null ;
                    if ($province && $factor) $en = $em->getRepository(Peace::class)->findOneBy(['province' => $province, 'year' => $row[4], 'factor' => $factor]);
                    if (!$en) {
                        $en = new Peace();
    ...
    #[Route('/api/peace/update', name: 'peace_is_update', methods: ['POST'])]
    public function Update(EntityManagerInterface $em, Request $request): Response
    {
        ...
        if ($email && $factorId) {
            ...
            if ($user && $factor) {
                if ($id) {
                    $val = $em->getRepository(Peace::class)->find($id);
                    if ($val) {
                        $val->setValue($value);
    ...
    #[Route('/api/peace/is-enabled', name: 'peace_is_enabled', methods: ['POST'])]
